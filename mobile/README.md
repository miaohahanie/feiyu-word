# 手机端（Android · M3 + M5 提醒）

本目录是「单词桌宠」手机端（Android）的 Flutter 源码与工程。

当前状态：**M2/M3/M5 已完成；新增“手机端添加单词 + 在线查词”，debug APK 构建通过**。
M3 包含：本地 SM-2 复习（跳过/结果页/本地历史）、今日复习统计、一致性单元测试。
提醒：每天 08:00–22:00 每 2 小时一轮（8/10/12/14/16/18/20/22 点）；前 3 次复习按 2h/4h/8h 短间隔提高密度。
手机端可通过“添加单词”手动填释义，或调用电脑端 /api/lookup 在线查词后加入词本。
